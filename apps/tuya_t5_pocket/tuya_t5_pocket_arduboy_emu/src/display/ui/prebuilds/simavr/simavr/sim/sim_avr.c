...
/* Optional platform-level callbacks for time and sleep */
static uint64_t (*g_platform_time_ns)(avr_t * avr) = NULL;
static void (*g_platform_sleep_us)(avr_t * avr, uint32_t sleep_us) = NULL;

void avr_set_platform_time_source(
        avr_t * avr,
        uint64_t (*time_ns_cb)(struct avr_t * avr))
{
    (void)avr;
    g_platform_time_ns = time_ns_cb;
}

void avr_set_platform_sleep_handler(
        avr_t * avr,
        void (*sleep_us_cb)(struct avr_t * avr, uint32_t sleep_us))
{
    (void)avr;
    g_platform_sleep_us = sleep_us_cb;
}
